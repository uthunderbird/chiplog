...
R16_DISPATCH_PRODUCTION_MANIFEST = replace(
    ...
    owners=tuple(
        replace(
            ...
            capability_ids=tuple(
                sorted(
                    (*owner.capability_ids, *(route.operation_id for route in _DISPATCH_V2_ROUTES))
                )
            ),
            public_operations=tuple(
                sorted(
                    (
                        *owner.public_operations,
                        *(route.operation_id for route in _DISPATCH_V2_ROUTES),
                    )
                )
            ),
            target_ids=("chiplog.capabilities.effects._dispatch_process:dispatch",),
        )
        if owner.owner_id == "effects"
        else owner
    ...
    routes=tuple(
        sorted(
            (*R16_PRODUCTION_MANIFEST.routes, *_DISPATCH_V2_ROUTES),
            key=lambda route: (route.callee_owner_id, route.operation_id),
        )
    ),
    leaves=tuple(
        sorted(
            (
                *R16_PRODUCTION_MANIFEST.leaves,
                LeafBinding(
                    "effects_transport",
                    "effects",
                    "chiplog.adapters.driven.effects_hermetic:HermeticEffectsProvider",
                ),
            ),
...
R16_DISPATCH_EVALUATION_MANIFEST = replace(
    R16_DISPATCH_PRODUCTION_MANIFEST, environment="evaluation"
)
...
_EXECUTION_TRANSITION_ROUTE = RoutedCallDecl(
...
_EXECUTION_LIFECYCLE_OPERATIONS = tuple(
    sorted((*_EXECUTION_OPERATIONS, _EXECUTION_TRANSITION_ROUTE.operation_id))
)
R14_EXECUTION_LIFECYCLE_PRODUCTION_MANIFEST = replace(
    R14_R17_EXECUTION_PRODUCTION_MANIFEST,
    manifest_version=12,
    owners=tuple(
        replace(
            owner,
            capability_ids=_EXECUTION_LIFECYCLE_OPERATIONS,
            public_operations=_EXECUTION_LIFECYCLE_OPERATIONS,
            target_ids=("chiplog.capabilities.agent_loop._execution_lifecycle_process:dispatch",),
        )
        if owner.owner_id == "agent_loop"
        else owner
        for owner in R14_R17_EXECUTION_PRODUCTION_MANIFEST.owners
    ),
    routes=tuple(
        sorted(
            (*R14_R17_EXECUTION_PRODUCTION_MANIFEST.routes, _EXECUTION_TRANSITION_ROUTE),
            key=lambda route: (route.callee_owner_id, route.operation_id),
        )
    ),
)
R14_EXECUTION_LIFECYCLE_EVALUATION_MANIFEST = replace(
