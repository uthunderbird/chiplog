...
class BrokerAuthorityLedger:
            ...
            if existing is not None:
                ...
                return (
                    OperationDisposition(
                        ...
                        recovery_obligation_id=None if existing[6] is None else str(existing[6]),
                    ),
                    False,
                )
            try:
                ...
                raise AuthorityLedgerConflict("nonce or response slot already consumed") from error
        return (
            OperationDisposition(
                tenant_id=token.tenant_id,
                operation_id=operation_id,
    ...
    def finish(self, disposition: OperationDisposition) -> OperationDisposition:
        ...
        if disposition.state == "DEFINITE":
            if disposition.result_bytes is None or disposition.recovery_obligation_id is not None:
                raise ValueError("definite disposition requires bytes and no recovery obligation")
        ...
        with sqlite3.connect(self._path) as connection:
            ...
            connection.execute(
                """UPDATE issued_operations
                   SET state = ?, result_digest = ?, result_bytes = ?, recovery_obligation_id = ?
