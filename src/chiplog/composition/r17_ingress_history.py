    ...
    *,
    ...
    try:
        with runtime._authority_gate().hold():
            ...
            windows = [
                (decision.prepared.predecessor_commitment, decision.resulting_commitment)
                for decision in pending
            ]
            windows.extend(
                (str(entry["predecessor"]), str(entry["resulting"])) for entry in runtime._pending()
            )
            for _, raw in runtime._pending_gate_publications():
                entry = json.loads(raw)
                if (
                    not isinstance(entry, dict)
                    or set(entry) != {"version", "predecessor", "resulting", "proposal"}
                    or type(entry["version"]) is not int
                    or entry["version"] != 1
                    or not isinstance(entry["predecessor"], str)
                    or not isinstance(entry["resulting"], str)
                ):
                    raise ValueError("unknown selected gate materialization")
                windows.append((entry["predecessor"], entry["resulting"]))
            if len(windows) > 1:
                raise ValueError("more than one selected publication requires recovery")
            if windows and not allow_pending:
                raise ValueError("selected publication requires recovery")
            with workspace_snapshot(runtime._database) as physical:
                ...
                if not windows and actual != anchor:
                    raise ValueError("physical custody state differs from independent anchor")
                if windows and (anchor not in windows[0] or actual not in windows[0]):
