...
#
# Отсюда же — что здесь НЕ гоняется. Гейт и его самотест тянут адаптеры стека,
# а те в живом проекте идут минутами (тем же доводом от них отказался
# dashboard.sh). Гейт отработает на коммите, которым ретро заканчивается.
set -eu
...
echo "   самотест гейта здесь не гоняется: он тянет адаптеры стека и всё равно"
