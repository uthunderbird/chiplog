...
def test_current_reference_revalidation_rejects_each_binding_substitution(
    tmp_path: Path,
    field: str,
) -> None:
    trust, _, _ = active(tmp_path)
    ...
    mutations = {
        "tenant_id": replace(original, tenant_id=TenantId("other")),
        "principal_id": replace(original, principal_id=PrincipalId("other")),
        "freshness_sequence": replace(original, freshness_sequence=original.freshness_sequence + 1),
        "contour": replace(original, contour="substituted"),
        "credential_head": replace(original, credential_head="substituted"),
        "session_head": replace(original, session_head="substituted"),
        "source_head": replace(original, source_head="substituted"),
        "trust_head": replace(original, trust_head="substituted"),
        "materialization_head": replace(original, materialization_head="substituted"),
        "peer_credential": replace(original, peer_credential="substituted"),
    }
    changed = mutations[field]
    result = trust.revalidate(
        TrustReferenceRevalidation(
            changed,
            "CREATE_INTENTION_LINE",
            RecordId(TenantId("tenant-1"), "line-1"),
    ...
    assert result.disposition in {"DENIED", "STALE"}
