...
import pytest
from tests.capabilities.agent_loop.test_fan_out_contracts import _request as shape_request
...
from chiplog.capabilities.agent_loop.recovery_contracts import Absent, NotApplicable, Present
...
def _ref(subject: str, raw: bytes) -> call.CallSubjectHead:
    digest = _sha(raw)
...
def _bind_run(value: fan.CapturedFanOutRequest, run: loop.RunRecord) -> fan.CapturedFanOutRequest:
    run = run.model_copy(update={"head": "pending"})
    run = run.model_copy(update={"head": "loop:" + _sha(run.canonical_bytes())})
...
def _bind_registry(
    ...
    return value.model_copy(
        update={
            ...
            "tool_registry_head": _ref(registry.registry_id, registry.canonical_bytes()),
        }
    )


def _raw(value: fan.CapturedFanOutRequest, raw: bytes) -> fan.CapturedFanOutRequest:
    ...
    return _bind_run(value, run)


async def _fixture(delivery: bool = False, complete: bool = False) -> fan.CapturedFanOutRequest:
    value = shape_request()
    ...
    entries = tuple(
        fan.FanOutToolPolicy(
            ...
            tool_schema=_ref(tool.schema_id, tool.canonical_bytes()),
            tool_policy=_ref("policy:" + tool.name, tool.name.encode()),
    ...
    value = _bind_registry(value, entries)
