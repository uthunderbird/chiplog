...
async def test_legacy_effect_history_refuses_dispatch_without_new_selection(tmp_path: Path) -> None:
    import sqlite3
    from contextlib import closing

    from chiplog.capabilities.agent_loop.contracts import LoopRejected
    from tests.composition.test_denial_runtime import _effect, _ingress

    database = tmp_path / "mixed.db"
    resources = HermeticDispatchResources(scenarios=("CONFIRM",), cap=1)
    response = json.loads(_response())
    response["tool_calls"].append({**response["tool_calls"][0], "call_id": "second"})

    def physical_effects() -> tuple[object, ...]:
        with closing(sqlite3.connect(database)) as connection:
            return (
                tuple(
                    connection.execute(
                        "SELECT * FROM records WHERE owner IN ('planning', 'effects') "
                        "ORDER BY tenant_id, record_id"
                    ).fetchall()
                ),
                tuple(
                    connection.execute(
                        "SELECT * FROM publications WHERE operation_kind LIKE 'effects.%' "
                        "ORDER BY tenant_id, operation_kind, idempotency_key"
                    ).fetchall()
                ),
            )

    async with open_dispatch_loop(
        database, resources=resources, responses=(json.dumps(response).encode(),)
    ) as loop:
        runtime, original = await _effect(loop)
        assert isinstance(runtime, R16DispatchRuntime)
        calls = runtime._loop_snapshot().records[-1].turns[-1].sealed_calls
        assert calls is not None
        assert len(calls) == 2 and all(call.state == "TERMINAL" for call in calls)
        assert len({call.proposal_id for call in calls}) == 2
        ingress = _ingress(original)
        cancelled = await runtime.dispose_effect("hermetic-ingress", "r", ingress)
        assert isinstance(cancelled, JournalSelectedPublication), cancelled
        assert cancelled.kind == "COMMITTED"
        with runtime._authority_gate().hold():
            selected_before = runtime._owner_decisions().snapshot()
        physical_before = physical_effects()
        with pytest.raises(LoopRejected, match="mixed legacy/v2 effects history is unsupported"):
            await runtime.preview_dispatch("r/turn/1/proposal/second")
        with runtime._authority_gate().hold():
            assert runtime._owner_decisions().snapshot() == selected_before
        assert physical_effects() == physical_before
        assert resources._provider.transfers == ()
    async with open_dispatch_runtime(database, resources=resources) as runtime:
        replay = await runtime.dispose_effect("hermetic-ingress", "no-current-run", ingress)
        assert isinstance(replay, JournalSelectedPublication), replay
        assert replay.kind == "EXACT_REPLAY"
        assert replay.complete_records == cancelled.complete_records
        assert physical_effects() == physical_before
        assert resources._provider.transfers == ()
