...
Fresh v2 operations require an effects history with no v1 records. Shared current
capture, including the final writer check, explicitly refuses an existing legacy
effects database before a new v2 selection. Mixed-version migration is not
implemented; already-mixed histories are unsupported and are not repaired by this
slice. Legacy-only cancellation replay and pure-v2 replay retain their existing paths.
