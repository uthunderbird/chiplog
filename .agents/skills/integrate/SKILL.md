...
# Project skill canon нельзя молча сливать или перезаписывать. Его обновление —
# отдельная интеграция с пофайловым решением владельца.
if [ -e .agents/skills ]
then
  echo 'СТОП: .agents/skills уже существует — сохрани существующий skill canon' >&2
  ABORTED_LOCK="$TXN_LOCK.aborted.$$"
  mv "$TXN_LOCK" "$ABORTED_LOCK"
  rm -f -- "$ABORTED_LOCK/state" "$ABORTED_LOCK/owner"
  rmdir "$ABORTED_LOCK"
...
git -C "$HB" ls-tree -rz --name-only "$RESOLVED" -- \
  .harness/scripts .harness/skills .agents/skills .githooks \
...
mkdir -p .agents
cp -R "$HB/export/.agents/skills" .agents/
...
`.agents/skills/` и `.githooks/` в `mktemp -d`, а отслеживаемые файлы восстанови только по поимённому списку
...
`.harness/skills/`, `.agents/skills/`, `.githooks/`, пустые каркасы
`.harness/{rules,reproducers,retro}/`.
